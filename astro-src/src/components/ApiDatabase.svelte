<script>
  ...
  onMount(async () => {
    ...
    try {
      const res = await fetch('http://10.10.7.121:8080/listApp', options);
      if (res.ok) {
        data = await res.json();
      } else {
    ...
    <ul style="list-style-type: none; padding: 0; max-width: 100%; overflow-x: hidden;">
      {#each data as item (item.id)}
        <li style="margin-bottom: 1rem; border-bottom: 1px solid black; display: flex;" key={item.id}>
          ...
          <div>
            <h3 style="margin-bottom: 0.5rem; max-width: 500px;">{item.hostname}</h3>
            <p style="margin-bottom: 0.5rem; max-width: 500px; height: 100px; overflow: hidden;">{item.version}</p>
